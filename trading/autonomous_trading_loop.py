...
class AutonomousTradingLoop:
    def __init__(self, 
                 ...
                 uncertainty_flagger=None,
                 milestone_system=None,
                 reporting_scheduler=None,
        ...
        self.uncertainty_flagger = uncertainty_flagger
        self.milestone_system = milestone_system
        self.reporting_scheduler = reporting_scheduler
    ...
    async def _poll_stock_prices(self, symbols: list, interval: float = 60):
        ...
        if signal:
            ...
            if self.enable_signal_persistence:
                if not self._passes_signal_persistence(symbol, signal):
                    self.loop_stats['hold_signals'] += 1
                    return
            if self.uncertainty_flagger:
                approved = self._apply_uncertainty_gate(signal, tick)
                if not approved:
                    self.loop_stats['hold_signals'] += 1
                    return
    ...
    async def _post_trade_updates(self, symbol: str, tick: Dict[str, Any], result: Dict[str, Any]):
        ...
        if result.get('side') == 'SELL' and 'pnl' in result:
            self.performance_monitor.record_trade_close(
                ...
                symbol=symbol,
                price=float(tick.get('close', 0.0) or 0.0),
                quantity=float(result.get('quantity', result.get('size', 1.0)) or 1.0),
        ...
        if result.get('side') == 'SELL' and 'pnl' in result:
            ...
            if self.milestone_system:
                milestone = self.milestone_system.check_milestones()
                if milestone:
                    logger.info(f"Milestone reached in loop: ${milestone.milestone_amount:,.0f}")
    ...
    def _run_reporting_tick(self):
        if not self.reporting_scheduler:
            return
        try:
            self.reporting_scheduler.tick()
        except Exception as exc:
            logger.warning(f"Reporting scheduler tick failed: {exc}")

    def _apply_uncertainty_gate(self, signal: Dict[str, Any], tick: Dict[str, Any]) -> bool:
        confidence = float(signal.get("confidence", 0.0) or 0.0)
        edge = float(signal.get("edge", 0.0) or 0.0)
        spread = float(signal.get("spread_pressure", 0.0) or 0.0)
        expected_move = float(signal.get("expected_move", 0.0) or 0.0)
        ...
        market_regime_confidence = regime_confidence_map.get(str(signal.get("regime", "")).lower(), 0.65)

        if volatility <= 0:
            volatility_score = 0.55
        elif self.min_acceptable_volatility <= volatility <= self.max_acceptable_volatility:
        ...
        historical_accuracy = float(self.performance_monitor.calculate_win_rate()) if self.performance_monitor.closed_trade_pnls else 0.5
        edge_coverage = 0.9 if expected_move > max(spread * 2.0, 1e-6) else 0.5

        assumptions = [
            f"Regime remains {signal.get('regime', self.current_regime)} for execution horizon",
            "Observed spread pressure remains stable during fill window",
        ]
        limitations = []
        if volatility_score < 0.5:
            limitations.append("Volatility regime is outside configured acceptance range")
        if edge < float(getattr(TradingConfig, "MIN_EDGE", 0.15)):
            limitations.append("Directional edge is near lower quality threshold")

        approval = self.uncertainty_flagger.score_plan(
            plan=f"Execute {signal.get('symbol', 'UNKNOWN')} {signal.get('decision', 'HOLD')}",
            ...
            edge_cases=["gap risk", "liquidity shock"],
        )

        signal["uncertainty_confidence"] = approval.confidence_score
        signal["uncertainty_risk_level"] = approval.risk_level
        signal["uncertainty_notes"] = approval.execution_notes
        signal["uncertainty_size_multiplier"] = approval.suggested_position_size

        if not approval.execution_approved:
            logger.info(f"Uncertainty gate HOLD for {signal.get('symbol', '')}: {approval.execution_notes}")
            return False

        allocation = float(signal.get("allocation", signal.get("size", 0.1)) or 0.1)
        scaled = allocation * float(approval.suggested_position_size)
        signal["allocation"] = max(0.0, min(1.0, scaled))
        return True
    ...
    def _passes_trade_quality(self, symbol: str, tick: Dict[str, Any], signal: Dict[str, Any], base_threshold: float) -> bool:
        ...
        if all(quality_checks):
            return True
        logger.info(
            f"Quality gate HOLD for {symbol}: expected_move={expected_move:.6f} spread={spread:.6f} "
