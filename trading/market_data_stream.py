...
class MarketDataStream:
        ...
        while self.running:
            for symbol in symbols:
                ...
                tick = {
                    ...
                    'high': float(prices[symbol] * 1.001),
                    'low': float(prices[symbol] * 0.999),
    ...
    async def stream_live_market_data(self, symbols: List[str]):
        if not self.broker_adapter:
            logger.warning('No broker adapter configured, using simulated market stream')
            await self.simulate_market_stream(symbols)
            return
        ...
        while self.running and attempt < self.max_reconnect_attempts:
            try:
                ...
                logger.warning(f'Live stream unavailable ({e}), falling back to simulation')
                await self.simulate_market_stream(symbols)
                return
            except Exception as e:
                attempt += 1
        ...
        if attempt >= self.max_reconnect_attempts:
            logger.error('Max reconnect attempts reached; switching to simulated stream')
            await self.simulate_market_stream(symbols)
