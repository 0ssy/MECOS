...
from trading import *
from trading.broker.yfinance_adapter import YFinanceAdapter
...
async def main():
    ...
    logger.info('Multi-Asset Autonomous Trading Platform (Paper Trading Mode)')
    logger.info('========================================')
    ...
    # Use yfinance adapter for paper trading (no API credentials needed)
    logger.info('Using YFinance adapter for real market data (paper trading)')
    stream.set_broker_adapter(YFinanceAdapter())
    ...
    try:
        ...
        logger.info(f'Final Equity: {final_stats["account"]["equity"]:.2f}')
